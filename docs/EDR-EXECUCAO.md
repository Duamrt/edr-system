...
- **Problemas de uso (medido runtime, desktop):** card = **143px** × 121 = ~17.000px de scroll · **121 botões "Excluir" visíveis** (um por card, admin) · fornecedor `<select>` com **~49 opções** → justifica **autocomplete** · sem ordenação. **390-reflow NÃO capturado** (resize travou em ~1478px; precisa do emulador/device mockup).
- **Exclusão:** travas PAGO/PENDENTE + confirmação de impacto + estorno ordenado lançamentos→distribuições→nota, mas **aborta-pra-frente SEM rollback** → **atomicidade NÃO confirmada** (deletes client-side sequenciais via `sbDelete`, sem transação/RPC única; CLAUDE.md registra "mover exclusão p/ RPC/Edge transacional" como pendência). Falha no passo C = estado PARCIAL. **NÃO mexer na lógica no redesign.** Filtros não persistem entre navegações.
