...
async function sbPostMinimal(t, b) {
  ...
  const r = await fetch(`${SUPABASE_URL}/rest/v1/${t}`, { method: 'POST', headers: {...hdrs, 'Prefer':'return=minimal'}, body: JSON.stringify(b) }); if (!r.ok) throw new Error(await r.text()); return true;
}
