    ...
    return `<div style="background:var(--bg2);border:1px solid var(--borda);border-left:3px solid ${corStatus};border-radius:10px;padding:16px;margin-bottom:8px;">
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:10px;flex-wrap:wrap;">
        ...
        <div style="display:flex;gap:6px;flex-shrink:0;">
          ${l.observacoes ? `<button onclick="verConversaLead('${l.id}')" style="background:rgba(255,255,255,0.04);border:1px solid var(--borda);color:var(--texto2);border-radius:6px;padding:5px 10px;font-size:10px;font-family:'Rajdhani',sans-serif;font-weight:700;cursor:pointer;">💬 CONVERSA</button>` : ''}
          ${l.status === 'novo' ? `<button onclick="atualizarLeadStatus('${l.id}','contatado')" style="background:rgba(59,130,246,0.08);border:1px solid rgba(59,130,246,0.2);color:#60a5fa;border-radius:6px;padding:5px 10px;font-size:10px;font-family:'Rajdhani',sans-serif;font-weight:700;cursor:pointer;">📞 CONTATADO</button>` : ''}
          ${l.status === 'contatado' ? `<button onclick="atualizarLeadStatus('${l.id}','convertido')" style="background:rgba(245,158,11,0.08);border:1px solid rgba(245,158,11,0.2);color:#f59e0b;border-radius:6px;padding:5px 10px;font-size:10px;font-family:'Rajdhani',sans-serif;font-weight:700;cursor:pointer;">✅ CONVERTIDO</button>` : ''}
          ${l.status !== 'descartado' ? `<button onclick="atualizarLeadStatus('${l.id}','descartado')" style="background:none;border:none;color:var(--texto3);cursor:pointer;font-size:12px;padding:4px;" title="Descartar">✕</button>` : `<button onclick="atualizarLeadStatus('${l.id}','novo')" style="background:rgba(34,197,94,0.08);border:1px solid rgba(34,197,94,0.15);color:#22c55e;border-radius:6px;padding:5px 10px;font-size:10px;font-family:'Rajdhani',sans-serif;font-weight:700;cursor:pointer;">↩ REABRIR</button>`}
        </div>
      </div>
    </div>`;
  }).join('');
