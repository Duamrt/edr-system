...
let obrasOrdem = 'data';
let obrasFiltroSemCodigo = false;
function obrasAtualizarOrdem() {
  ...
  const elCod = document.getElementById('obras-ord-codigo');
  if (elCod) elCod.classList.toggle('ativo', obrasFiltroSemCodigo);
}
...
function filtrarLanc() {
  ...
  // Filtro sem código (toggle independente)
  if (obrasFiltroSemCodigo) lista = lista.filter(l => !/^\d{4,6}\s*[·-]/.test(l.descricao||''));
  // Ordenação
...
async function editarEtapaLanc(lancId) {
  ...
  modal.innerHTML = `<div style="background:var(--bg2);border:1px solid var(--borda2);border-radius:14px;padding:28px;max-width:440px;width:100%;">
    ...
    <select id="sel-nova-etapa" style="width:100%;padding:14px 12px;border-radius:8px;background:#111;color:#fff;border:1px solid #555;font-size:15px;font-weight:600;margin-bottom:20px;color-scheme:dark;">
