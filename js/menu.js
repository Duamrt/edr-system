...
function resetarOrdemMenu() {
  ...
  const nav = document.getElementById('main-nav');
  const sidebarBottom = nav.querySelector('.sidebar-bottom');
  const ordemPadrao = ['dashboard','obras','estoque','notas','form','creditos','diarias','catalogo','relatorio','custos','caixa','contas-pagar','banco','setup'];
...
function setView(v) {
  ...
  closeBnavMore();
  syncBnav(v);
  const views = ['dashboard','obras','estoque','notas','form','creditos','setup','catalogo','banco','relatorio','diarias','custos','leads','caixa','contas-pagar'];
  views.forEach(name => {
